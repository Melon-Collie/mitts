...
# geometry utilities (_blade_y_local, _stick_horiz) live on SkaterController
# because they are also used by SkaterIKCoordinator.
...
func apply_slapper_blade_position() -> void:
	...
	var current_blade_y: float = lerpf(_controller._blade_y_local(), _controller.slapper_wind_up_height, wind_up_t)
	var pos := Vector3(
	...
	if _controller.has_puck:
		clamped_heel = _controller._clamp_blade_from_goalies(clamped_heel)
	...
	var clamped_contact: Vector3 = _controller._clamp_blade_from_net(contact_world)
...
func apply_wrister_follow_through() -> void:
	...
	var stick_horiz: float = _controller._stick_horiz()
	...
	intended_target.y = _controller._blade_y_local() + arc * _controller.wrister_follow_through_blade_lift
	...
	var net_world: Vector3 = _controller._clamp_blade_from_net(_skater.upper_body_to_global(local_target))
...
func apply_slapper_follow_through() -> void:
	...
	var blade_pos := Vector3(
		...
		lerpf(_controller.slapper_wind_up_height, _controller._blade_y_local(), smoothstep(0.0, 1.0, t)),
	...
	blade_pos = _skater.upper_body_to_local(_controller._clamp_blade_from_net(_skater.upper_body_to_global(blade_pos)))
