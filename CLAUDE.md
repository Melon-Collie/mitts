...
- **Slot colors still drive off-screen indicators and the scoreboard badges:** `PlayerRules.slot_color()` and the team-color badges are intentionally untouched — they need disambiguation between teammates and the on-ice overlay redesign doesn't replace that role. Revisit if the redesign makes the on-ice system carry enough identity that slot colors can come out of the off-screen arrows / scoreboard too.
