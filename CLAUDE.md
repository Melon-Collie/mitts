...
- **HUD-on-ice redesign needs playtest tuning:** Slot color rings under skaters were replaced with a single ice-blue overlay system (`MenuStyle.HUD_ICE` / `HUD_LINE_THIN` / `HUD_OPACITY` and the new `CHARGE_*` palette in `Scripts/ui/menu_style.gd`). Each skater now carries: a thin ice-blue slot ring, a concentric charge ring (yellow→orange fill clockwise from 12 o'clock, red pulse at full, red flash on cancel — driven by a small `ShaderMaterial`), a chevron to the right of the player name when `is_elevated`, an outline arrow originating at the slapshot zone (shaft + shoulder + arrowhead — closed except at the base) pointing along `slapper_aim_dir`, and a billboarded `Label3D` player name (no outline). Charge ring + slapshot arrow are local-only (gated on `show_one_timer_indicator` in `SkaterController`); slot ring + chevron + name show on every skater. The slapshot reticle (`set_slapper_indicator`) was recolored to ice blue and `set_slapper_indicator_ready` / `update_slapper_indicator_window` are now no-ops — the previous green-ready / red-countdown states were dropped per redesign. Tuning targets after first playtest: ring radii (`Skater.RING_OUTER_R`, `CHARGE_RING_INNER_R`/`OUTER_R`), chevron / arrow proportions inside `_create_chevron_mesh` / `_create_arrow_mesh`, name-label `pixel_size` (currently 0.005) and the screen-down offset (`RING_OUTER_R + 0.18` — both the name Label3D and the chevron are `top_level = true` and re-positioned each tick using camera-aware screen-down/right axes computed in `_hud_screen_down_xz`, so they always sit below + to-the-right of the skater regardless of `PlayerPrefs.attack_up`), opacity (`MenuStyle.HUD_OPACITY = 0.70`), and full-charge pulse Hz (`_CHARGE_FULL_PULSE_HZ = 3.0`).
