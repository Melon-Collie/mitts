...
**World state wire format:** PackedByteArray quantized encoding (~200 bytes vs ~520 bytes unquantized, ~60% reduction). Skater: 35 bytes (pos s16/s8/s16 @1cm, vel 3×s16 @0.1 m/s, blade/top_hand 3×s16 @1cm each, facing u16, upper_body_rot f32, lp_ts f32, flags u8, charge u8). Puck: 13 bytes. Goalie: 10 bytes. Encode/decode in `WorldStateCodec`; typed state objects (`SkaterNetworkState` etc.) unchanged elsewhere.
