...
- **Puck interaction detection uses swept-puck vs static-blade:** `PuckInteractionRules.check_pickup` and `check_poke` sweep the puck (`puck_prev → puck_curr`) but treat the blade as a static point sampled at the end of the tick. The correct test is segment-vs-segment (capsule vs capsule): sweep both the puck and the blade (`blade_prev → blade_curr`). When the puck is nearly stationary the test degenerates to point-vs-point, missing fast blade swings that pass through the pickup zone within a single tick. Fix: add `Skater.get_prev_blade_contact_global()` (stored at the top of `_physics_process` before movement), update `PuckInteractionRules` to a segment-segment distance test, and pass `blade_prev` alongside `blade_pos` at all call sites in `PuckController._check_interactions`. The lag-comp path in `game_manager.gd` can get `blade_prev` from the state buffer snapshot one tick earlier (`rewind_time - 1/240`) — the same pattern already used for `puck_prev`.
