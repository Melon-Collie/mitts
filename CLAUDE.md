...
**Rates:** inputs 60 Hz unreliable (client → host), batched as last 12 inputs per packet (PackedByteArray, 279 bytes at 12 inputs); world state 40 Hz unreliable (host → clients); events reliable RPCs.

**Skaters:** `LocalController` predicts + reconciles (snap to server state → replay unacknowledged inputs kinematically → immediate snap, no gradual correction). Each replay iteration advances `global_position += velocity * delta` so blade IK and facing evaluate against a moving body. Reconcile saves and restores only the narrow shot-state fields (`_state`, follow-through timers, one-timer window) — visual and charge fields use replay output directly. Mouse position is seeded from the first replayed input at replay start (and from the last at replay end) so wrister-aim charge accumulation is deterministic across reconcile. Server authority on shot state: if `server_state.shot_state` differs from the pre-replay client state after replay, server wins and both `_state` and `_charge_distance` are overwritten. Client inputs are delayed `INPUT_DELAY_FRAMES = 2` physics ticks (≈8ms at 240Hz) before being applied; stamped with `estimated_host_time()` at apply-time so the reconcile echo and RemoteController sort order stay correct. The pending input queue is drained on `is_movement_locked()` and `is_input_blocked()` phases so stale inputs don't flood through when a phase lifts. `RemoteController` drives from input on host via a queue (all inputs from each batch are sorted by `host_timestamp` and popped one per physics tick; `just_pressed` flags cleared on the fallback so they don't re-fire; queue capped at 120 entries ≈ 500ms to prevent clock-drift lag accumulation); interpolates buffered snapshots (100ms delay) on clients, with blade and top-hand positions extrapolated using the body velocity field (not a derived delta — dividing position deltas by client receive-time gaps amplifies jitter into enormous blade jumps). `_apply_state_to_skater` sets facing and upper-body before blade so shaft orientation is correct each tick.

**Puck:** three client-side modes — local carrier (pinned to blade), trajectory prediction (Jolt runs client-side after release; each 40Hz broadcast computes `latency_corrected.position = state.position + state.velocity * rtt_s` for full-RTT correction, but only hard-snaps position+velocity when divergence exceeds `trajectory_hard_snap_threshold` (1.5m) — no soft blend, because RTT jitter ±20ms at 20m/s = ±0.4m positional noise and blending toward a noisy target creates visible snapback every broadcast tick), interpolation (100ms buffer, position only). `_carrier_peer_id` is managed exclusively by reliable RPCs, never by world state, to avoid unreliable ordering conflicts. Pickup claims use lag compensation: client sends a reliable `receive_pickup_claim` RPC with `host_timestamp` and `rtt_ms`; host rewinds `StateBufferManager` to `host_timestamp − rtt/2`, reads `blade_contact_world` (world-space mid-blade, host-only non-serialized field on `SkaterNetworkState`) from the rewound snapshot for segment-segment distance testing, and either grants the pickup, squirts the puck on a contested claim (two claims within 50ms, contest timer runs in `_physics_process` for determinism), or drops it as stale/invalid. `rtt_ms` sent is the raw unaveraged latest sample (`latest_rtt_ms` from `ClockSync`) rather than the smoothed average, so rewind depth tracks the actual current round-trip rather than a lagging average. Shot release uses lag-compensated placement: the `release_puck` RPC carries `host_timestamp` and `rtt_ms`; on the host, the shooter's latest `blade_contact_world` (≈ client blade at release time, since RemoteController lags by RTT/2) is advanced kinematically by `direction * power * rtt_half` before `puck.release()` fires, aligning the host's starting puck position with the client's trajectory prediction.
...
**World state wire format:** Single flat PackedByteArray per broadcast (~279 bytes at 6 players + 2 goalies, well under the 1392-byte ENet MTU). Layout: u16 ws_sequence + u8 skater_count, then per-skater [u32 peer_id + 35B state + u8 queue_depth], then 13B puck, then u8 goalie_count + n×8B goalies, then 4×u8 + u16 game state (score0, score1, phase, period, time_remaining). Skater 35B: pos s16/s8/s16@1cm, vel 3×s16@0.02m/s, blade/top_hand 3×s16@1cm each, facing u16, upper_body_rot s16@π/32767 rad, lp_ts f32, flags u8 (shot_state[3:0]+ghost[4]), charge u8. Puck 13B: pos s16/s8/s16@1cm, vel 3×s16@0.02m/s, carrier_peer_id s16. Goalie 8B: pos_x s16@1cm, pos_z s16@1cm, rot_y s16@π/32767, state u8, fho u8. Encode/decode in `WorldStateCodec`; typed state objects (`SkaterNetworkState` etc.) unchanged elsewhere.

**Body check hit crediting:** `HitTracker.on_hit` is reached via two paths — host-side physics contact (authoritative) and a lag-compensated client claim (`send_hit_claim` RPC with `host_timestamp`/`rtt_ms`; host rewinds and checks distance). A 0.5s per-pair cooldown deduplicates both paths. Body check impulse replay: `Skater` emits `body_check_impulse_applied(impulse: Vector3)` with the total velocity delta from `_resolve_player_collisions()` each tick — capturing both the attacker rebound AND the victim transfer from the remote skater's own `_resolve_player_collisions` call. `LocalController` connects to this signal (not the old `body_checked_player`, which only contained attacker rebound) and injects the complete delta into the kinematic reconcile replay at the matching `host_timestamp`, so replay accurately reproduces the post-collision trajectory. Body check VFX fires client-side from the sim (SkaterVFX connects to `body_checked_player` on every skater); remote-checks-local fires because the local CharacterBody3D is a real physics body that the remote skater's `move_and_slide()` detects.
...
| `tests/unit/game/` | GUT tests for application-layer collaborators: `ShotOnGoalTracker` (pending-shot FSM, assists, SOG dedup), `WorldStateCodec` (stats round-trip), `SlotSwapCoordinator` (request validation), `ClockSync` (RTT math, readiness, outlier dropping), `InputState` (serialization round-trip, field-count sentinel), `SkaterNetworkState` (round-trip preserves all wire fields, array length sentinel, host-only fields not serialized). |
...
- **Shot release snapback (host-side Jolt bug):** When the puck is released from carry, `puck.release()` fires from B_{M-1} (the previous frame's pinned position) rather than B_M (current blade), because `Puck._physics_process` pins to `carrier.get_blade_contact_global()` on the same tick that `release()` is called. Combined with a Jolt freeze→unfreeze transition artifact, this causes a 1-frame snap at release that is visible on the host and unrelated to networking. Deferred to a dedicated shot mechanics pass.
- **Netcode deferred items (out of scope for current pass):** Adaptive broadcast rate — reduce from 40Hz during dead-puck phases to save bandwidth. Priority-based state updates — skaters with recent puck interaction or active reconcile error get higher broadcast priority. Host migration — transfer network authority to a surviving peer on host disconnect. None of these are blocking for playtest builds.
