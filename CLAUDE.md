...
**Body check hit crediting:** `HitTracker.on_hit` is reached via two paths — host-side physics contact (authoritative) and a lag-compensated client claim (`send_hit_claim` RPC with `host_timestamp`/`rtt_ms`; host rewinds and checks distance). A 0.5s per-pair cooldown deduplicates both paths. Body check impulse replay: `LocalController` captures the self-velocity impulse from `body_checked_player` at the contact tick and injects it into the kinematic reconcile replay at the matching `host_timestamp`, so replay accurately reproduces the post-collision trajectory without oscillation.
