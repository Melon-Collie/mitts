...
- **Shoulder-to-hand drop causes arm stretch at max backhand:** `shoulder_height` (0.35 m upper-body-local) positions the arm's anchor up on the torso; `hand_rest_y` (0.0) sits at waist. Vertical drop at rest ≈ 0.35 m. At max backhand extension (horizontal hand displacement = `rom_backhand_reach_max` = 0.7 m), shoulder-to-hand distance reaches ≈ 0.78 m, within the 0.84 m arm reach (`upper_arm_length + forearm_length`). If `rom_backhand_reach_max` is raised, verify the diagonal distance stays under the arm sum to avoid visible stretch.
