...
- **Top-hand IK is Phase 1 only:** `TopHandIK.solve` operates in the horizontal plane with `hand_rest_y` fixed. Fixed stick length (shaft only) means the blade heel can't come in closer to the body than `stick_horiz` (~0.89 m) without the hand moving vertically; the contact point (mid-blade) sits ~0.15 m further forward. Phase 2: let hand Y rise/dip so the stick can swing through the vertical plane, restoring tight-in blade positions. Also: slapper blade pose uses a fixed offset (`slapper_blade_x`, `slapper_blade_z`) and doesn't respect `stick_length` exactly — tune those values (or route through IK) once the baseline feels right.
