...
- **Facing and upper-body rotation use linear lerp in interpolation:** `RemoteController._interpolate` and `StateBufferManager._interpolate_skater` lerp `upper_body_rotation_y` with `lerpf` and `facing` with `lerp_facing` (angle-aware, correct). `upper_body_rotation_y` should also use `lerp_angle` to handle the ±π wrap correctly. Both fields could use Hermite interpolation (matching the position/velocity spline) for smoother remote skater movement during high-latency intervals, but this requires storing angular velocity in `SkaterNetworkState`.
- **Input batch size is fixed, not adaptive:** Clients always send the last 12 (or 24 during packet loss) inputs per batch regardless of actual measured loss rate. A proper adaptive scheme would scale redundancy with `get_peer_loss_rate()` — low loss → smaller batches → less bandwidth; high loss → more redundancy. Not worth the complexity until packet loss is a real observed problem in playtesting.
