...
- **Goalie reactive saves not yet implemented:** glove saves, shoulder/body saves, and stick poke coverage are planned. The stick is currently disabled (`stick_enabled = false`) — it can be re-enabled once it has proper positional behavior.
...
- **Pending netcode work:** reconcile position blend instead of instant snap (`docs/specs/NETCODE_AUDIT_2026.md` #3); `WorldStateCodec` bounds checking on malformed packets (#4); `NetworkSim` one-sided jitter (#5); Hermite interpolation for remote skaters (#6); session-relative timestamps for long-session f32 precision (`docs/netcode-improvements-plan.md` #4).
