...
**`facing` in `SkaterNetworkState` is `Vector2` (XZ packed as XY), not `Vector3`.** `BufferedStateInterpolator.lerp_facing` takes and returns `Vector2`; it extracts the angle via `atan2(x, y)` and reconstructs with `Vector2(sin, cos)`. Do not pass a `Vector3` facing direction to this helper.
...
- **Pending netcode work:** `WorldStateCodec` byte-level bounds checking on malformed packets (`docs/specs/NETCODE_AUDIT_2026.md` #4); session-relative timestamps for long-session f32 precision (`docs/netcode-improvements-plan.md` #4); adaptive broadcast rate during dead-puck phases.
...
- **No TTL on client input replay history:** Inputs that are permanently dropped (not just reordered) are never explicitly ACKed by the host. The client advances `last_processed_host_timestamp` from incoming world states, which implicitly expires confirmed inputs, but under sustained extreme packet loss (>20%) the replay buffer can grow stale entries faster than they're cleared. If this becomes a problem, add a cap on replay history depth keyed to `max_rtt * 240` frames.
- **Facing and upper-body rotation interpolation:** `upper_body_rotation_y` now uses `lerp_angle` in both `RemoteController._interpolate` and `StateBufferManager._interpolate_skater` (±π wrap fixed). `facing` uses the angle-aware `lerp_facing` helper (correct). Both fields could use Hermite interpolation (matching the position/velocity spline) for smoother remote skater movement during high-latency intervals, but this requires storing angular velocity in `SkaterNetworkState`.
- **Input batch size is partially adaptive:** Batch size switches between 12 (normal) and 24 (if `get_peer_loss_rate() > 10%`) per packet. The 1-second measurement window only has ~40 samples at 40 Hz and resets to zero each second, making the loss signal noisy — a burst of 5 drops reads as 12.5% even on an otherwise clean connection, causing the batch size to oscillate on marginal connections (8–12% loss). A proper fix would apply EMA to the loss rate before using it for batch sizing and use a sliding window rather than a resetting one.
- **No dev-build replay divergence detection:** `LocalController.reconcile` silently accepts any server correction. In a debug build it would be valuable to hash the predicted vs. server state at reconcile time and log/assert when they diverge by more than a threshold — this would catch non-determinism bugs (missing board clamp, uninjected impulse, etc.) that currently only show up as subtle visual snapping.
...
- **Reconcile invariants live in docs, not code:** The subtle constraints in `LocalController.reconcile` (mouse seeding, shot-state save/restore, board clamping, locked-phase guard) are documented in CLAUDE.md but not in comments near the code. A future editor is unlikely to read this file before touching that function.
