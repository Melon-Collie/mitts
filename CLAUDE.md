...
| `domain/rules/bottom_hand_ik.gd` | Reactive 1-bone IK for the bottom (off-stick) hand. Given a grip target along the shaft (caller derives it from `top_hand.lerp(blade, grip_fraction)`), places the hand against an asymmetric ROM anchored at the opposite shoulder — loose toward the blade (same-side), tight toward the top hand (cross-body). A smoothstep release blends the hand back to a rest pose at the bottom shoulder as the cross-body angle approaches its ROM max, giving a natural one-handed backhand when the stick swings beyond reach. Never influences blade or top-hand placement. |
...
- **Bottom-hand ROM values are first-principles baselines:** `bottom_hand_grip_fraction` (0.25), `bh_rom_same_side_angle_max_deg` (110°), `bh_rom_cross_body_angle_max_deg` (60°), `bh_rom_same_side_reach_max` (0.60 m), `bh_rom_cross_body_reach_max` (0.30 m), and `bh_release_angle_band_deg` (15°) are anatomical defaults and will need playtest tuning. The release band controls how early the bottom hand lets go toward neutral as the stick swings into extreme backhand — widen it for an earlier release, narrow it to keep the hand attached longer. Note the bottom hand is rendered via the same two-bone arm meshes as the top, so `upper_arm_length` / `forearm_length` apply to both; the bottom elbow uses a mirrored pole (`-arm_pole_local.x`).
