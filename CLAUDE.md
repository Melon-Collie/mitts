...
- **`MenuStyle` should migrate to a `Theme` resource:** The static-factory pattern (`apply_button`, `apply_toggle`, `apply_slider`, `apply_line_edit`, `apply_tab_button`) bypasses Godot's built-in theme system. Every styled control has to remember to call the right `apply_*` (which has caused recurring drift bugs — e.g. unstyled lobby OptionButtons + OT CheckButton + loading-screen Cancel button), every control allocates its own duplicate styleboxes, and there's no helper for `SpinBox`, `CheckButton`, or future controls without writing more factories. The Godot-native approach: build `Resources/MenuTheme.tres` in the editor with stylebox/color entries for `Button`, `OptionButton`, `LineEdit`, `HSlider`, `SpinBox`, `CheckButton`; declare `ToggleButton` and `TabButton` as theme type variations (Godot's CSS-class equivalent); set it as the Project Settings default theme. Then most `apply_*` calls disappear (controls style themselves on instantiation) and `apply_toggle` collapses to `btn.theme_type_variation = &"ToggleButton"; btn.toggle_mode = true`. The migration is mostly editor work — translating each existing factory into a serialized resource — and needs visual verification since procedural styleboxes can drift slightly from a serialized version.
