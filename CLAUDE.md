...
- **Four impact sounds need audio files:** `puck_boards.wav`, `puck_goalie.wav`, `puck_post.wav`, `body_check.wav` are wired in `SoundManager` and `game_manager.gd` but the files don't exist yet — drop them into `res://Sounds/` to activate. Board hits are broadcast to all clients via `NetworkManager.notify_board_hit` (unreliable RPC). Note: `puck_touched_post` / `puck_post.wav` fires for any contact with `HockeyGoal` geometry (posts, crossbar, back boards) — not post-only.
