...
**Trajectory prediction uses hard-snap only, no soft blend.** Each broadcast computes `latency_corrected.position = state.position + state.velocity * rtt_s` (full-RTT forward correction) and hard-snaps position + velocity only when divergence exceeds `trajectory_hard_snap_threshold` (1.5m). RTT jitter ±20ms at 20m/s = ±0.4m positional noise; blending toward that noisy target creates visible snapback every broadcast tick.
...
- **Pending netcode work:** session-relative timestamps for long-session f32 precision — after ~4 hours `Time.get_ticks_usec() / 1e6` as f32 loses sub-ms precision; needs a session-start offset. WorldStateCodec skater/goalie per-field bounds checking (currently only an aggregate header size guard). Out-of-order world-state rejection in `RemoteController` is silent — no telemetry counter for states dropped OOO. `StateBufferManager.get_state_at()` silently returns the oldest available state when the requested timestamp predates the 3-second buffer; a `push_warning` on that path would surface clock drift early.
...
- **Input replay history cap is RTT-adaptive:** Cap is `clamp(2 × RTT_frames, 48, 480)` so under sustained high loss the buffer can't grow unboundedly. Inputs older than the cap are dropped from the front; the `last_processed_host_timestamp` filter from incoming world states remains the primary expiry mechanism.
- **Facing and upper-body rotation interpolation:** `upper_body_rotation_y` uses `lerp_angle` in both `RemoteController._interpolate` and `StateBufferManager._interpolate_skater` (±π wrap fixed). `facing` uses the angle-aware `lerp_facing` helper; codec convention is compass `atan2(x,y)` / `Vector2(sin,cos)` throughout. Both fields could use Hermite interpolation for smoother remote skater movement during high-latency intervals, but this requires storing angular velocity in `SkaterNetworkState`.
- **Input batch size is partially adaptive:** Batch size switches between 12 (normal) and 24 (if `get_peer_loss_rate() > 10%`) per packet. The loss rate is EMA-smoothed (α=0.3, ~3s memory) so a single bad second doesn't immediately flip the threshold. Remaining concern: the 1-second measurement window only has ~40 samples at 40 Hz, so the signal is still fairly noisy on marginal connections. A sliding window rather than a resetting one would improve accuracy further.
- **Goalie network state includes velocity:** `GoalieNetworkState` has `velocity_x`/`velocity_z` fields (derived from frame-over-frame position deltas in `GoalieController._update_position`). GOALIE_BLOCK_SIZE is 12 bytes. During packet-loss extrapolation the goalie dead-reckons using this velocity rather than freezing in place. The `extrapolation_friction` export on `PuckController` applies matching velocity decay during puck interpolation extrapolation — default 0.5 m/s/s; tune against observed Jolt deceleration.
- **Adaptive world-state broadcast rate:** Host drops to 5 Hz during dead-puck phases (GOAL_SCORED, FACEOFF_PREP, END_OF_PERIOD, GAME_OVER) via `NetworkManager.set_broadcast_rate(hz)` called from `GameManager._on_phase_for_broadcast_rate`. Recovers ~40% of session bandwidth during pauses. Reverts to `Constants.STATE_RATE` (40 Hz) on live phases.
- **StateBufferManager uses binary search:** `_find_bracket` / `_find_bracket_puck` / `_find_bracket_goalie` binary-search the monotone timestamp ring buffer (O(log 720) ≈ 10 comparisons) instead of O(n) linear scan. Fill counts (`_skater_counts`, `_puck_count`, `_goalie_counts`) are maintained alongside ring-buffer write pointers.
- **Reconcile divergence detection:** `LocalController.reconcile` measures pre-replay position divergence from the server state and records it via `NetworkTelemetry.record_prediction_divergence`. In `OS.is_debug_build()` builds, divergence > 10 cm emits a `push_warning` with the magnitude and replay input count — helps surface non-determinism (missing board clamp, wrong friction, uninjected impulse) that would otherwise only show up as subtle visual snapping. The `prediction_divergence_avg` metric is published by `NetworkTelemetry` each second. `RemoteController` OOO drops are now counted in `NetworkTelemetry.ooo_drops_per_sec`.
...
- **Reconcile invariants:** Mouse seeding, shot-state save/restore, and board clamping all have inline comments explaining their rationale. The locked-phase guard comment is brief but present. Remaining doc-only invariant: the forward-prediction cap in `RemoteController` (`extrapolation_max_ms`) is not commented inline.
...
- **Puck reconciliation has a hard cliff at 1.5 m:** Below the `trajectory_hard_snap_threshold` nothing happens to position; above it both position and velocity hard-snap. The discontinuity creates a visible pop exactly at the threshold. A graduated response — soft position blend under 0.3 m, velocity-only snap at 0.3–1.5 m, hard snap above — would eliminate the cliff, but requires reworking the trajectory prediction path in `PuckController.apply_state`.
