...
- **Eight impact/event sounds need audio files:** `puck_boards.wav`, `puck_goalie.wav`, `puck_post.wav`, `puck_goal_body.wav`, `puck_deflection.wav`, `puck_strip.wav`, `period_buzzer.wav`, `body_check.wav` are wired in `SoundManager` and `game_manager.gd` but the files don't exist yet — drop them into `res://Sounds/` to activate. Board, goal-body, deflection, and strip hits are broadcast to all clients via unreliable RPCs. `puck_post.wav` fires only on pipe contacts (posts, crossbar, mouth corner bends); skirt and net panels trigger `puck_goal_body.wav`. `period_buzzer.wav` plays at every period transition and game over. `puck_deflection.wav` covers all `puck_touched_loose` events (blade redirects and body blocks); `puck_strip.wav` covers poke checks and body-check strips.
