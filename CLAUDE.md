...
- **Five impact sounds need audio files:** `puck_boards.wav`, `puck_goalie.wav`, `puck_post.wav`, `puck_goal_body.wav`, `body_check.wav` are wired in `SoundManager` and `game_manager.gd` but the files don't exist yet — drop them into `res://Sounds/` to activate. Board hits and goal-body hits are broadcast to all clients via unreliable RPCs. `puck_post.wav` fires only on pipe contacts (posts, crossbar, mouth corner bends); skirt and net panels trigger `puck_goal_body.wav` instead.
