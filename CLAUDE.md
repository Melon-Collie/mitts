...
- **Four impact sounds need audio files:** `puck_boards.wav`, `puck_goalie.wav`, `puck_post.wav`, `body_check.wav` are wired in `SoundManager` and `game_manager.gd` but the files don't exist yet — drop them into `res://Sounds/` to activate. Board hit detection fires on both host (always) and client (only during puck trajectory prediction); full networked board hit RPCs are not implemented.
