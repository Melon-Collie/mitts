...
**Reconcile saves and restores only narrow shot-state fields** (`_state`, follow-through timers, one-timer window). Visual and charge fields come from replay output. Server authority on shot state: if `server_state.shot_state` differs after replay, server wins — `_state` and `_charge_distance` are overwritten. **Two symmetric guards protect in-flight shot transitions:**
...
- **Slot colors removed:** `PlayerRules.slot_color()` is gone. Off-screen indicators (`Scripts/ui/off_screen_player_indicators.gd`) draw a team-primary-colored arrow with the player's jersey number painted just inside the arrow base via `draw_string` (using `record.text_color` / `record.text_outline_color` so the digits match the on-jersey number). The scoreboard's #/POS/PLAYER columns are now uniformly white. Team disambiguation between teammates relies on jersey numbers + names; if playtest shows that's not enough, revisit. Tuning targets in `off_screen_player_indicators.gd`: `_NUMBER_FONT_SIZE` (18 px at full scale), `_NUMBER_MIN_FONT_SIZE` (12 px floor when arrows shrink at distance), `_NUMBER_OFFSET_FACTOR` (0.95 of arrow size — places the number just inside the arrow base toward screen center).
